function [ ] = eventTrackingCompartment( ip )
%% Event tracking ? Compartment mode
...
%   Create variables for even metadata parameters
    ...
    tsField = cell2mat(allEvents(iEvents, 5)); 
...
%   Load eventROI
    ...
    for ii = 1:length(fn)
        if isnan(roiMetadata.(char(fn(ii)))) == 1
            roiMetadata = rmfield(roiMetadata,(char(fn(ii))));
        end
    end
    
    cd (char(ip.fdp))
    [corrROI, corrMetadata] = chromCorrection(ip,eventROI,roiMetadata,nChannels,emWavelengths);
...
%               Confirm if true t0 or not: Check confirmTimes number of timepoints post-Event
                ...
                for tConf = (conf_t0+1):(conf_t0+ip.confirmTimes)
                      if isDism == 1 || tConf > nTimes
                         break
                      end
                      findStackMax = 0;
                      for iPlane = 1:nPlanes     
                      findPlaneMax = 0;
                      data = eventROI(:,:,iPlane,tConf,1);
...
%                         Constrain scanning to within eventROI                          
    ...
    if t0found == 1
       t0 = conf_t0;
        ...
        cd (char(pw));
        destination = 'compartmentTrackingdata';
...
%   Extract up to and including t0, using stationary data extraction
%   (iTime-(t0-samplePre)+1)) generates numbers 1 to last
    for iTime = (t0-ip.samplePre):t0
      writeTo = iTime-(t0-ip.samplePre)+1;
      zMin = zCC_t0-ip.zSearchDistance;
      zMax = zCC_t0+ip.zSearchDistance;
      if zMin < 1
         zMin = 1;
      end
      if zMax > nPlanes
      ...
      for iPlane = zMin:zMax
          data = eventROI(:,:,iPlane,iTime,1);
          subset = data((yCC_t0-oUL):(yCC_t0+oDR), (xCC_t0-oUL):(xCC_t0+oDR));
          maskedObject = subset.*objectMask;
          meanMaskValue = sum(sum(maskedObject)) / ...
          length(objectMask(objectMask==1));  
...
%         Extract primary channel donut
          subset = data((yCC_t0-dUL):(yCC_t0+dDR), (xCC_t0-dUL):(xCC_t0+dDR));
...
%         Save fitted mask to tracks-struct
          tracks((yCC_t0-dUL):(yCC_t0+dDR), (xCC_t0-dUL):(xCC_t0+dDR), zCC_t0, iTime) = mergeMask;
      end

        vectordata.object_mean.primary(1,writeTo) = stackdata.object_mean.primary(zCC_t0,writeTo);
        vectordata.object_mean.secondary(1,writeTo) = stackdata.object_mean.secondary(zCC_t0,writeTo);
        vectordata.donut_mean.primary(1,writeTo) = stackdata.donut_mean.primary(zCC_t0,writeTo);
        vectordata.donut_mean.secondary(1,writeTo) = stackdata.donut_mean.secondary(zCC_t0,writeTo);
        
        vectordata.object_median.primary(1,writeTo) = stackdata.object_median.primary(zCC_t0,writeTo);
        vectordata.object_median.secondary(1,writeTo) = stackdata.object_median.secondary(zCC_t0,writeTo);
        vectordata.donut_median.primary(1,writeTo) = stackdata.donut_median.primary(zCC_t0,writeTo);
        vectordata.donut_median.secondary(1,writeTo) = stackdata.donut_median.secondary(zCC_t0,writeTo);
        
        vectordata.object_STD.primary(1,writeTo) = stackdata.object_STD.primary(zCC_t0,writeTo);
        vectordata.donut_STD.primary(1,writeTo) = stackdata.donut_STD.primary(zCC_t0,writeTo);
        vectordata.object_STD.secondary(1,writeTo) = stackdata.object_STD.secondary(zCC_t0,writeTo);
        vectordata.donut_STD.secondary(1,writeTo) = stackdata.donut_STD.secondary(zCC_t0,writeTo);
        
        trackingdata.coordinates.data(writeTo,:) = [xCC_t0 yCC_t0 zCC_t0 iTime];
        if nChannels == 3
           vectordata.object_mean.tertiary(1,writeTo) = stackdata.object_mean.tertiary(zCC_t0,writeTo);
           vectordata.donut_mean.tertiary(1,writeTo) = stackdata.donut_mean.tertiary(zCC_t0,writeTo);
           
           vectordata.object_median.tertiary(1,writeTo) = stackdata.object_median.tertiary(zCC_t0,writeTo);
           vectordata.donut_median.tertiary(1,writeTo) = stackdata.donut_median.tertiary(zCC_t0,writeTo);
           
           vectordata.object_STD.tertiary(1,writeTo) = stackdata.object_STD.tertiary(zCC_t0,writeTo);
           vectordata.donut_STD.tertiary(1,writeTo) = stackdata.donut_STD.tertiary(zCC_t0,writeTo);
        end
            
    end % iTime = t0-samplePre:t0
    
%   Extract from and including (t0+1) to and including (t0+samplePost),
